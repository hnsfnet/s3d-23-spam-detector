"""SQLite storage for prediction history, user feedback, and training logs.

Three tables are maintained:

* ``predictions`` -- every classification made via the API.
* ``feedback`` -- user corrections with an ``applied`` flag.
* ``trainings`` -- one row per (re)training event.

The module uses the standard-library ``sqlite3`` and is thread-safe by opening a
fresh connection per call, which fits Flask's default threaded server.
"""
...
class PredictionDatabase:
    """SQLite-backed storage for predictions, feedback, and training history."""
    ...
    def save_feedback(
        ...
        if predicted_label not in config.VALID_LABELS:
            raise ValueError(f"predicted_label must be one of {config.VALID_LABELS}")
        if correct_label not in config.VALID_LABELS:
            raise ValueError(f"correct_label must be one of {config.VALID_LABELS}")
